...
1. 对齐维度： 从两个张量的尾部（即最后一个维度）开始对齐。如果维度数量不一致，则在较少维度的张量前面补充维度1。

1. 匹配维度： 在对齐的过程中，如果两个张量在某一维度的长度相同，或者其中一个张量的长度为1，则可以在该维度上进行运算。

1. 扩展维度： 如果在某一维度上一个张量的长度为1，另一个张量的长度为n（n > 1），则在计算时，长度为1的那个张量会沿该维度扩展为长度为n。这一过程称为广播。

1. 不匹配时出错： 如果在任何维度上两个张量的长度既不相同又都不为1，那么操作会出错，无法广播。
