...
3. **模拟（Simulation）**：此步骤执行所选策略，并使用公式（1）确定状态序列，通过公式（3）计算所选策略的累积奖励 \( <(℘) \)。图2展示了在图1所示选择和扩展步骤之后的模拟步骤示例。

4. **反向传播（Backpropagation）**：作为单次MCTS迭代的最后一步，算法通过选定的策略进行反向传播，并更新树策略节点的属性。图3展示了此步骤的示例。为明确起见，反向传播算法在树策略的所有节点上执行以下任务：
   - 将计算出的累积奖励 \( <(℘) \) 添加到树策略中每个节点的当前存储奖励 \( r_a \)。
   - 将 \( n_a \) 和 \( N \) 均增加一个单位。
...
以下是论文中“IV. Level-K博弈论的整合”（IV. INTEGRATION OF LEVEL-K GAME THEORY）部分的完整翻译：
...
\[
<^l(℘_i) = \sum_{k=0}^m d^k \cdot r_i(a_k, s_k | ℘_{\text{others}})
\]

其中，\( ℘_{\text{others}} \) 为自车对所有周围车辆的预测策略。

以下是论文中“V. 结果与讨论”（V. RESULTS AND DISCUSSION）部分的完整翻译：
